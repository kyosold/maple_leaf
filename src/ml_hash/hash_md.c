...
{
    ml_hash_bin2hex(md5str, digest, 16);
    md5str[32] = '\0';
}
 ...
 */
void ml_md5_init(ml_md5_ctx * context)
{
...
/* {{{ ml_md5_update 
   ...
   context.
 */
void ml_md5_update(ml_md5_ctx * context, const unsigned char *input, unsigned int inputLen)
{
 ...
 */
void ml_md5_final(unsigned char digest[16], ml_md5_ctx * context)
{
    ...
    ml_md5_update(context, PADDING, padLen);

    /* Append length (before padding) */
    ml_md5_update(context, bits, 8);
 ...
 */
void ml_md4_init(ml_md4_ctx * context)
{
    context->count[0] = context->count[1] = 0;
    /* Load magic initialization constants.
     */
...
/* {{{ ml_md4_update
   ...
   context.
 */
void ml_md4_update(ml_md4_ctx * context, const unsigned char *input, unsigned int inputLen)
{
    unsigned int i, index, partLen;
...
/* {{{ ml_md4_final
   ...
   the message digest and zeroizing the context.
 */
void ml_md4_final(unsigned char digest[16], ml_md4_ctx * context)
{
    unsigned char bits[8];
    unsigned int index, padLen;
    ...
    index = (unsigned int) ((context->count[0] >> 3) & 0x3f);
    padLen = (index < 56) ? (56 - index) : (120 - index);
    ml_md4_update(context, PADDING, padLen);

    /* Append length (before padding) */
    ml_md4_update(context, bits, 8);
...
{
    memset(context, 0, sizeof(ml_md2_ctx));
}

static void MD2_Transform(ml_md2_ctx *context, const unsigned char *block)
{
...
void ml_md2_update(ml_md2_ctx *context, const unsigned char *buf, unsigned int len)
{
...
void ml_md2_final(unsigned char output[16], ml_md2_ctx *context)
{
