...
{
    ml_hash_bin2hex(sha1str, digest, 20);
    sha1str[40] = '\0';
}
...
/* {{{ ml_sha1_update
   ...
   context.
 */
void ml_sha1_update(ml_sha1_ctx * context, const unsigned char *input,
...
void ml_sha1_final(unsigned char digest[20], ml_sha1_ctx *context)
{
    ...
    ml_sha1_update(context, PADDING, padLen);

    /* Append length (before padding) */
    ml_sha1_update(context, bits, 8);
...
void make_sha224_digest(char *sha1str, unsigned char *digest)
{
    ml_hash_bin2hex(sha1str, digest, 28); 
    sha1str[56] = '\0';
}

void make_sha256_digest(char *sha1str, unsigned char *digest)
{
    ml_hash_bin2hex(sha1str, digest, 32); 
    sha1str[64] = '\0';
}

void make_sha512_digest(char *sha1str, unsigned char *digest)
{
    ml_hash_bin2hex(sha1str, digest, 64); 
    sha1str[128] = '\0';
}
 ...
 */
void ml_sha256_init(ml_sha256_ctx * context)
{
    context->count[0] = context->count[1] = 0;
    /* Load magic initialization constants.
     */
 ...
 */
void ml_sha224_init(ml_sha224_ctx * context)
{
    context->count[0] = context->count[1] = 0;
    /* Load magic initialization constants.
     */
...
/* {{{ ml_sha224_update
   ...
   context.
 */
void ml_sha224_update(ml_sha224_ctx * context, const unsigned char *input, unsigned int inputLen)
{
    unsigned int i, index, partLen;
...
/* {{{ ml_sha224_final
   ...
   the message digest and zeroizing the context.
 */
void ml_sha224_final(unsigned char digest[28], ml_sha224_ctx * context)
{
    unsigned char bits[8];
    unsigned int index, padLen;
    ...
    index = (unsigned int) ((context->count[0] >> 3) & 0x3f);
    padLen = (index < 56) ? (56 - index) : (120 - index);
    ml_sha224_update(context, PADDING, padLen);

    /* Append length (before padding) */
    ml_sha224_update(context, bits, 8);
...
/* {{{ ml_sha256_update
   ...
   context.
 */
void ml_sha256_update(ml_sha256_ctx * context, const unsigned char *input, unsigned int inputLen)
{
    unsigned int i, index, partLen;
...
/* {{{ ml_sha256_final
   ...
   the message digest and zeroizing the context.
 */
void ml_sha256_final(unsigned char digest[32], ml_sha256_ctx * context)
{
    unsigned char bits[8];
    unsigned int index, padLen;
    ...
    index = (unsigned int) ((context->count[0] >> 3) & 0x3f);
    padLen = (index < 56) ? (56 - index) : (120 - index);
    ml_sha256_update(context, PADDING, padLen);

    /* Append length (before padding) */
    ml_sha256_update(context, bits, 8);
 ...
 */
void ml_sha384_init(ml_sha384_ctx * context)
{
    context->count[0] = context->count[1] = 0;
    /* Load magic initialization constants.
     */
...
/* {{{ ml_sha384_update
   ...
   context.
 */
void ml_sha384_update(ml_sha384_ctx * context, const unsigned char *input, unsigned int inputLen)
{
    unsigned int i, index, partLen;
...
/* {{{ ml_sha384_final
   ...
   the message digest and zeroizing the context.
 */
void ml_sha384_final(unsigned char digest[48], ml_sha384_ctx * context)
{
    ...
    ml_sha384_update(context, PADDING, padLen);

    /* Append length (before padding) */
    ml_sha384_update(context, bits, 16);
 ...
 */
void ml_sha512_init(ml_sha512_ctx * context)
{
    context->count[0] = context->count[1] = 0;
    /* Load magic initialization constants.
     */
...
/* {{{ ml_sha512_update
   ...
   context.
 */
void ml_sha512_update(ml_sha512_ctx * context, const unsigned char *input, unsigned int inputLen)
{
    unsigned int i, index, partLen;
...
/* {{{ ml_sha512_final
   ...
   the message digest and zeroizing the context.
 */
void ml_sha512_final(unsigned char digest[64], ml_sha512_ctx * context)
{
    unsigned char bits[16];
    unsigned int index, padLen;
    ...
    index = (unsigned int) ((context->count[0] >> 3) & 0x7f);
    padLen = (index < 112) ? (112 - index) : (240 - index);
    ml_sha512_update(context, PADDING, padLen);

    /* Append length (before padding) */
    ml_sha512_update(context, bits, 16);
